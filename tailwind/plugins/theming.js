...
// -----------------------------------------------------------------------------------------------------
// @ ONTRIAL TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
const theming = plugin.withOptions((options) => ({
	...
	addComponents(
		_.fromPairs(_.map(options.themes, (theme, themeName) => [
			...
			))), (value, key) => [[`--ontrial-${e(key)}`, value], [`--ontrial-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
		])),
	);
	...
	const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) => {
		...
		const background = theme(`ontrial.customProps.background.${colorScheme}`);
		const foreground = theme(`ontrial.customProps.foreground.${colorScheme}`);
		...
		return {
			[(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
				...
				..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--ontrial-${e(key)}`, value], [`--ontrial-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
				..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--ontrial-${e(key)}`, value], [`--ontrial-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
			},
		};
	});
			...
			theme: {
				extend: {
					...
					colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
						[name, `rgba(var(--ontrial-${name}-rgb), <alpha-value>)`],
						[`on-${name}`, `rgba(var(--ontrial-on-${name}-rgb), <alpha-value>)`],
					]))),
				},
				ontrial: {
					customProps: {
