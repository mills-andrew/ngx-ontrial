...
@Component({
	selector: 'ontrial-vertical-navigation',
...
export class OntrialVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy {
	...
	@HostBinding('class') get classList(): any {
		/* eslint-disable @typescript-eslint/naming-convention */
		return {
			'ontrial-vertical-navigation-animations-enabled': this._animationsEnabled,
			[`ontrial-vertical-navigation-appearance-${this.appearance}`]: true,
			'ontrial-vertical-navigation-hover': this._hovered,
			'ontrial-vertical-navigation-inner': this.inner,
			'ontrial-vertical-navigation-mode-over': this.mode === 'over',
			'ontrial-vertical-navigation-mode-side': this.mode === 'side',
			'ontrial-vertical-navigation-opened': this.opened,
			'ontrial-vertical-navigation-position-left': this.position === 'left',
			'ontrial-vertical-navigation-position-right': this.position === 'right',
		};
		/* eslint-enable @typescript-eslint/naming-convention */
	}
	...
	ngAfterViewInit(): void {
		...
		setTimeout(() => {
			...
			if (!this._navigationContentEl.nativeElement.classList.contains('ps')) {
				...
				const activeItem = this._navigationContentEl.nativeElement.querySelector('.ontrial-vertical-navigation-item-active');
			...
			// Otherwise
				...
				this._ScrollbarDirectives.forEach((ScrollbarDirective) => {
					...
					ScrollbarDirective.scrollToElement('.ontrial-vertical-navigation-item-active', -120, true);
				});
			}
		});
	}
	...
	private _showOverlay(): void {
		...
		this._overlay!.classList.add('ontrial-vertical-navigation-overlay');

		// Add a class depending on the transparentOverlay option
		if (this.transparentOverlay) {
			this._overlay!.classList.add('ontrial-vertical-navigation-overlay-transparent');
		}
	...
	private _showAsideOverlay(): void {
		...
		this._asideOverlay!.classList.add('ontrial-vertical-navigation-aside-overlay');
