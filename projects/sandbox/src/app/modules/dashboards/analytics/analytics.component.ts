...
export class AnalyticsComponent implements OnInit, OnDestroy {
	...
	/**
	 * 
	 * @returns Get the series value for gitHubIssues
	 */
	getSeriesValue(value: ApexAxisChartSeries | ApexNonAxisChartSeries, index: any): ApexAxisChartSeries | ApexNonAxisChartSeries {
		const seriesValue = value?.[index];

		// If seriesValue is a number, return it as an ApexNonAxisChartSeries
		if (typeof seriesValue === 'number') {
			return [seriesValue];
		}

		// If seriesValue is defined and is an array, return it as is
		if (Array.isArray(seriesValue)) {
			return seriesValue;
		}

		// If value is an array and the first element is a number, return as an ApexNonAxisChartSeries
		if (Array.isArray(value) && typeof value[0] === 'number') {
			return [value[0]];
		}

		// If value is an array and the first element is an object, return it
		if (Array.isArray(value) && typeof value[0] === 'object') {
			return [value[0]];
		}

		// Handle the case when both seriesValue and value[0] are undefined
		// You can decide on a default value or handle this case differently.
		if (seriesValue === undefined && (value[0]) === undefined) {
			// Return a default value or handle differently.
			// For example, you could return an empty ApexNonAxisChartSeries:
			return [];
		}

		// If all else fails, throw an error
		throw new Error('Invalid series value.');
	}
