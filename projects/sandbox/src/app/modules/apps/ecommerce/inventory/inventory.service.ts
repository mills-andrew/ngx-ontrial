...
export class InventoryService {
	// Private
	private _brands: BehaviorSubject<InventoryBrand[] | null> = new BehaviorSubject<InventoryBrand[] | null>(null);
	...
	private _tags: BehaviorSubject<InventoryTag[] | null> = new BehaviorSubject<InventoryTag[] | null>(null);
	private _vendors: BehaviorSubject<InventoryVendor[] | null> = new BehaviorSubject<InventoryVendor[] | null>(null);
			...
			switchMap(products => this._httpClient.patch<InventoryProduct>('api/apps/ecommerce/inventory/product', {
				...
				switchMap(updatedProduct => this.product$.pipe(
					...
					map(item => {
						...
						throw new Error("Product not found or ID mismatch");
					})
				)),
			)),
		);
	...
	deleteTag(id: string): Observable<boolean> {
		return this.tags$.pipe(
			take(1),
			switchMap(tags => this._httpClient.delete<boolean>('api/apps/ecommerce/inventory/tag', { params: { id } }).pipe(
				map((isDeleted) => {
					if (isDeleted) {
