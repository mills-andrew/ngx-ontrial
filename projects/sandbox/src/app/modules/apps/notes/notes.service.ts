...
export class NotesService {
	...
	updateNote(note: Note): Observable<Note> {
		...
		if (updatedNote.labels.length) {
			updatedNote.labels = updatedNote.labels.map((label: { id: any; }) => label.id);
		}
	...
	deleteNote(note: Note): Observable<boolean> {
		if (note.id == null) {
			throw new Error('Note ID is undefined or null');
		}

		return this._httpClient.delete<boolean>('api/apps/notes', { params: { id: String(note.id) } }).pipe(
			map((isDeleted) => {
				this.getNotes().subscribe();
				return isDeleted;
			}),
