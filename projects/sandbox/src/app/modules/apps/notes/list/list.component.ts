...
import { MasonryComponent } from '@ngx-ontrial/material';
...
@Component({
	...
	standalone: true,
	imports: [MatSidenavModule, MatRippleModule, MatDialogModule, NgClass, MatIconModule, NgIf, NgFor, MatButtonModule, MatFormFieldModule, MatInputModule, MasonryComponent, AsyncPipe],
})
		...
		this.notes$ = combineLatest([this._notesService.notes$, this.filter$, this.searchQuery$]).pipe(
			...
			map(([notes, filter, searchQuery]) => {
				...
				if (searchQuery) {
					...
					filteredNotes = filteredNotes.filter((note: { title: any; content: any; }) => note.title!.toLowerCase().includes(searchQuery!) || note.content!.toLowerCase().includes(searchQuery!));
				}
				...
				filteredNotes = filteredNotes.filter((note: { archived: boolean; }) => note.archived === isArchive);

				// Filter by label
				if (filter.startsWith('label:')) {
					const labelId = filter.split(':')[1];
					filteredNotes = filteredNotes.filter((note: { labels: any; }) => !!note.labels!.find((item: { id: any; }) => item.id === labelId));
				}
