...
export class TasksService {
		...
		return this.tags$.pipe(
			...
			switchMap(tags => this._httpClient.patch<Tag>('api/apps/tasks/tag', {
				...
				map((updatedTag) => {
					...
					//Error out if we try updating the tag when we have no reference to the tags array.
					if (tags === null || tags === undefined) {
						throw new Error("Could not reference the tags array.");
					}
		...
		return this.tags$.pipe(
			...
			switchMap(tags => this._httpClient.delete('api/apps/tasks/tag', { params: { id } }).pipe(
				...
				switchMap(isDeleted => this.tasks$.pipe(
					take(1),
					map((tasks) => {
						//Error out if we try updating the tag when we have no reference to the tasks array.
						if (tasks === null || tasks === undefined) {
							throw new Error("Could not reference the tasks array.");
						}
	 ...
	 * @param task
	 */
	updateTask(id: string, task: Task): Observable<Task | null> {
		return this.tasks$
			.pipe(
				take(1),
				switchMap(tasks => this._httpClient.patch<Task>('api/apps/tasks/task', {
					...
					switchMap(updatedTask => this.task$.pipe(
						take(1),
						filter((item: Task | null) => item !== null && item.id === id),
						tap(() => {
